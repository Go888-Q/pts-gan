...
class spatial_attn_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = self.spatial(torch.cat([avg_out, max_out], dim=1))
        return x * attn
...
class ca_layer(nn.Module):
    ...
    def forward(self, x):
        attn = self.channel(self.avg_pool(x))
        return x * attn


class CAFF(nn.Module):
    def __init__(self, in_ch, hidden_ch, text_ch=768):
    ...
    def forward(self, x, text_features):
        ...
        if text_features.dim() > 2:
            text_features = text_features.view(text_features.shape[0], text_features.shape[1])
        if text_features.shape[0] != image_features.shape[0]:
            text_features = text_features[:1].expand(image_features.shape[0], -1)
        text_gate = self.text_proj(text_features.float()).view(image_features.shape[0], -1, 1, 1)
        return image_features * text_gate


class TextualSemanticAlignment(CAFF):
    pass
...
class DualAttentionRouting(DualAttention):
    pass
...
class DEBlock(nn.Module):
    def __init__(self, in_ch, out_ch, text_ch=768):
        super(DEBlock, self).__init__()
        self.conv = nn.Conv2d(in_ch, out_ch, 3, 1, 1)
        self.CAFF = TextualSemanticAlignment(in_ch, in_ch*8, text_ch=text_ch)
        # self.DCBlk = DCBlock(out_ch)
        self.conv2 = nn.Conv2d(text_ch, out_ch, 3, 1, 1)
...
class D_Block(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, stride, padding, bn=True, text_ch=768):
        super(D_Block, self).__init__()
        self.bn = bn
        self.conv = nn.Conv2d(in_ch, text_ch, kernel_size, stride, padding, bias=False)
        self.out = nn.Conv2d(text_ch, out_ch, kernel_size, stride, padding, bias=False)
        ...
        if bn==True:
            self.batchnorm = nn.BatchNorm2d(text_ch)
        else:
            self.batchnorm = None
...
class SemanticFeatureExtraction(D_Block):
    pass
    
class NetD(nn.Module):
    def __init__(self, text_ch=768):
        super(NetD, self).__init__()

        self.block0 = SemanticFeatureExtraction(1, 48, 3, 1, 1, bn=False, text_ch=text_ch)#128
        self.block1 = SemanticFeatureExtraction(49, 49, 3, 1, 1, text_ch=text_ch)#64
        self.block2 = SemanticFeatureExtraction(50, 50, 3, 1, 1, text_ch=text_ch)#32
...
class Dual(nn.Module):
    def __init__(self, 
        ...
        self.fuse1 = DEBlock(dim, dim, text_ch=text_ch)
        self.fuse1_b = DEBlock(dim, dim, text_ch=text_ch)
        self.fuse2 = DEBlock(dim*2, dim*2, text_ch=text_ch)
        self.fuse2_b = DEBlock(dim*2, dim*2, text_ch=text_ch)
        self.fuse3 = DEBlock(dim*4, dim*4, text_ch=text_ch)
        self.fuse3_b = DEBlock(dim*4, dim*4, text_ch=text_ch)
        
        self.spatial_routing_encoder_level1 = DualAttentionRouting( atom_dim, dim) 
        self.spatial_routing_encoder_level2 = DualAttentionRouting( atom_dim*2, int(dim*2**1))
        self.spatial_routing_encoder_level3 = DualAttentionRouting( atom_dim*4, int(dim*2**2))
        self.spatial_routing_encoder_level1_b = DualAttentionRouting( atom_dim, dim) 
        self.spatial_routing_encoder_level2_b = DualAttentionRouting( atom_dim*2, int(dim*2**1))
        self.spatial_routing_encoder_level3_b = DualAttentionRouting( atom_dim*4, int(dim*2**2))


        
        self.channel_routing_latent = DualAttentionRouting( atom_dim*8, int(dim*2**3))
        self.channel_routing_latent_b = DualAttentionRouting( atom_dim*8, int(dim*2**3))

        self.channel_routing_decoder_level3 = DualAttentionRouting( atom_dim*4, int(dim*2**2)) 
        self.channel_routing_decoder_level2 = DualAttentionRouting( atom_dim*2, int(dim*2**1)) 
        self.channel_routing_decoder_level1 = DualAttentionRouting( atom_dim*2, int(dim*2**1)) 
        self.channel_routing_decoder_level3_b = DualAttentionRouting( atom_dim*4, int(dim*2**2)) 
        self.channel_routing_decoder_level2_b = DualAttentionRouting( atom_dim*2, int(dim*2**1)) 
        self.channel_routing_decoder_level1_b = DualAttentionRouting( atom_dim*2, int(dim*2**1)) 
...
def Net_G(text_ch=768):
    return Dual(text_ch=text_ch)

def Net_D(text_ch=768):
    return NetD(text_ch=text_ch)
